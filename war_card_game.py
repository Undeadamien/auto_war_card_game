...
class Game:
    ...
    def __init__(self):
        ...
        self.black_pile_pos = (self.size[0]//2-self.card_size[0]//2, self.size[1]//2-self.card_size[1])
        self.black_pile = []

        self.red_deck_pos = (self.size[0]-self.card_size[0], self.size[1]-self.card_size[1])
    ...
    def prepare_deck(self):
        ...
        for row in range(4):
            for col in range (13):
                ...
                value, image = col+1, (col*self.card_size[0], row*self.card_size[1], self.card_size[0], self.card_size[1])

                self.cards.append(self.Card(value, image))

        shuffle(self.cards)

        self.black_deck, self.red_deck = self.cards[:len(self.cards)//2], self.cards[len(self.cards)//2:]
    ...
    def draw_decks(self):
        ...
        for pos, color in [[self.black_deck_pos, 2], [self.red_deck_pos, 3]]:
            self.surface.blit(self.cards_sprite, pos, (14*self.card_size[0], color*self.card_size[1], self.card_size[0], self.card_size[1]))

    def draw_piles(self):

        for pile, pos, color in [[self.black_pile, self.black_pile_pos, 2], [self.red_pile, self.red_pile_pos, 3]]:
...
while game.running:
    ...
    if game.giving_cards or game.removing_cards:
        ...
        if (len(game.black_pile)%2 == 0 and len(game.red_pile)%2 == 0) or game.black_pile[-1].value == game.red_pile[-1].value:
            ...
            game.sprites.append([game.black_deck_pos, game.black_pile_pos, "black"])
            game.sprites.append([game.red_deck_pos, game.red_pile_pos, "red"])
            ...
            if game.black_pile[-1].value < game.red_pile[-1].value:
                ...
                game.sprites.append([game.black_pile_pos, game.red_deck_pos, "black"])
                game.sprites.append([game.red_pile_pos, game.red_deck_pos, "red"])
                ...
                game.sprites.append([game.red_pile_pos, game.black_deck_pos, "red"])
                game.sprites.append([game.black_pile_pos, game.black_deck_pos, "black"])
                game.removing_cards = True
